...
class PackBufferTest : public ::testing::TestWithParam<std::tuple<std::size_t, float>> {
 protected:
  bool pack_test_impl(std::size_t size, float tol) {
    ...
    auto tbuffer = hsys::TaskBuffer(
      ...
      [&](const auto& pack){ hsys::run_pack_dynpack(pack); }
    );
    // clang-format on
...
TEST_P(PackBufferTest, pack_test) {
