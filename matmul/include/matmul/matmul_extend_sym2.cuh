...
__global__ void matmul_extend_sym2(
  ...
  const std::size_t j = 2 * tile_size * blockIdx.x + threadIdx.x;
  ...
  AtomT acc1{0}, acc2{0}, acc3{0}, acc4{0};
  ...
  for (std::size_t v = 0; v < k; v += tile_size) {
    ...
    for (std::size_t t = 0; t < tile_size; ++t) {
      ...
      acc3 += tile_a2[ty][t] * tile_b1[t][tx];
      acc4 += tile_a2[ty][t] * tile_b2[t][tx];
    }
  ...
  if (i + tile_size < m and j < n) c(i + tile_size, j) = acc3;
  if (i + tile_size < m and j + tile_size < n) c(i + tile_size, j + tile_size) = acc4;
}
